...
// Replaces one or more consecutive non-alphabetical characters with a single underscore
...
function findBestMatchOrUseOriginal(item, threshold) {
...
async function processLabels(labels, threshold) {
  ...
  const newThesaurusItems = normalizedEmbeddings.map((item) => {
    const similarFoundOrOriginalItem = findBestMatchOrUseOriginal(
      item,
      threshold
    );
...
app.post("/thesaurus", async (req, res) => {
  try {
    const { labels, threshold } = req.body;
